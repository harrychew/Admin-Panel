...
if(!isset($_SESSION['admin_id'])){
	...
	if(isset($_GET['delete_transporter'])){
		$delete_id=$_GET['delete_transporter'];
		$delete_transporter="delete from transporters where Transporter_id='$delete_id'";
		$run_delete =mysqli_query($con,$delete_transporter);
		if($run_delete){
			echo "<script>alert('A Transporter has been deleted!')</script>";
			echo "<script>window.open('transporters.php?admin_id=$admin_id','_self')</script>";
		}
	}
}
?>